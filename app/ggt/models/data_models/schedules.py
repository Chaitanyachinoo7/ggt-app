...
def create_schedule_entry(location_id, start_dt, end_dt, duration, status):
    try:
        sql = """
        INSERT INTO schedules (location_id, start_dt, end_dt, duration, status) 
        VALUES (%s, %s, %s, %s, %s)
        """
        ...
        log_generic(type="error", function='__insert_record_schedules', location_id=location_id, start_dt=start_dt, end_dt=end_dt, duration=duration, status=status, error=err)
        return None
...
def get_schedule_generation_rules_by_location_id(location_id):
    try:
        ...
        log_generic(type="error", function='__read_record_schedule_generation_rules_by_location_id', location_id=location_id, error=err)
        return None



#TODO: apply Group Code filter
def get_available_dates(group_code):
    try:
        ...
        log_generic(type="error", function='get_dates_available', error=err)
        return None
...
def get_available_times(location_id, date):
    try:
        sql = """
        SELECT distinct time(start_dt) as start_time, id, start_dt, end_dt, status 
        FROM schedules 
        WHERE location_id = %s 
...
def get_slot_information(slot_id):
    try:
        sql = """
        SELECT id, location_id, start_dt, end_dt, duration, status, appointment_id 
        FROM schedules 
        WHERE id = %s
        """
        val = (slot_id,)
...
def update_slot_information(slot_id, appointment_id):
    ...
    try:
        ...
        log_generic(type="error", slot_id=slot_id, appointment_id=appointment_id, function='__update_slot_information', error=err)
        return None
