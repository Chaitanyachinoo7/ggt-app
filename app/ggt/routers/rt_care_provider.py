...
from ggt.models.workflow_models.care_provider_field_flow import (
    ...
    provider_rollback_to_pending_task, call_patient, _get_provider_processing_list)

router = APIRouter()


@router.post("/provider_rollback_to_pending_task", dependencies=[Security(authorize_user, scopes=[p.PROVIDER_ROLLBACK_TO_PENDING_TASK])])
...
@router.post("/end_consultation", dependencies=[Security(authorize_user, scopes=[p.PROVIDER_COMPLETE_TASK])])
async def api_provider_complete_task(complete_task: CompleteNoteReq):
    return provider_complete_task(complete_task)


@router.post("/call_patient", dependencies=[Security(authorize_user, scopes=[p.CALL_PATIENT])])
async def api_call_patient(call_request: CallReq):
    return call_patient(call_request)


@router.post("/begin_consultation", dependencies=[Security(authorize_user, scopes=[p.LOCK_PROVIDER_TASK])])
...
@router.get("/get_provider_processing_list", dependencies=[Security(authorize_user, scopes=[p.ANONYMOUS])])
async def _api_get_provider_processing_list():
    return _get_provider_processing_list()
