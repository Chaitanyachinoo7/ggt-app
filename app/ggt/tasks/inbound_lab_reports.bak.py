...
def temp():
    from ggt.lib.adapters.s3_adapter import get_list_of_files, copy_file_from_s3_to_s3
    files = get_list_of_files('ggt-sftp','mawdpath/prod/results/')
    ...
    for filename in files:
        try:
            ...
            if requisition_id is None:
                print_warning('skipping {}'.format(filename))
                continue

            sql = """
            INSERT IGNORE INTO mawdpath_inbound_data
            (
                requisition_id,
                order_number,
                status,
    ...
    try:
        ...
        for local_file_path in glob.iglob('{}/*.pdf'.format(local_download_path), recursive=True):
            ...
            try:
                ...
                if __destination_filename:
                    if shutil.copyfile(local_file_path, '{}/{}'.format(local_backups_path, __destination_filename)):
                        print('cp {} {}/{}'.format(local_file_path, local_backups_path, __destination_filename))
                    else:
                        print('copy failed for {} -> {}/{}'.format(local_file_path, local_backups_path, __destination_filename))
                    '''
                    if file_exists_in_files_in_remote_storage_cache(__destination_filename):
                        filename = extract_filename(local_file_path)
...
def extract_report_info_mawd_v2(file_path):
    filename = None
    vial_id = None
    order_number = None
    test_result = None
