...
if env == "dev" or env == None:
	create_permission_url = create_permission_url.format(development_api_id)
if env == "prod":
	create_permission_url = create_permission_url.format(production_api_id)

body = {'scopes': permissions }
body = json.dumps(body)
current_status = requests.get(create_permission_url, headers=headers)
...
if temp.lower() == "y":
	r = requests.patch(create_permission_url, data=body, headers=headers)
else:
